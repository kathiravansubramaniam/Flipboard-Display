...
/** Slack sends `:name:` shortcodes; convert to Unicode before layout. */
function expandSlackEmoji(s) {
    return emojify(String(s ?? '').trim());
}
...
function layoutMessage(raw) {
    const text = expandSlackEmoji(typeof raw === 'string' ? raw : '');
...
module.exports = { layoutMessage, expandSlackEmoji };
